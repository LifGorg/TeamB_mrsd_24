...
function GeofenceHumanPanel({ context }: { context: PanelExtensionContext }): ReactElement {
  ...
  const plannedPathMarkersRef = useRef<L.CircleMarker[]>([]);
  ...
  const [waypointYaw, setWaypointYaw] = useState<number>(0.0); // User-configurable yaw in degrees (0-360, 0=North)
  ...
  const ensureAdvertised = () => {
    ...
    try {
      ...
      context.advertise(CLICKED_OBSTACLE_TOPIC, "sensor_msgs/NavSatFix");
      ...
      console.log("[GeofenceMap] ✓ Advertised:", SELECTED_WAYPOINT_TOPIC, CLICKED_OBSTACLE_TOPIC, GEOFENCE_MAPPING_ALTITUDE_TOPIC, SURVEY_ALTITUDE_TOPIC);
    } catch (error) {
  ...
  const publishSelectedWaypoint = useCallback((lat: number, lon: number, alt?: number, yaw?: number) => {
    ...
    // Use provided altitude and yaw or the user-configured values
    const altitude = alt ?? waypointAltitude;
    const yawDegrees = yaw ?? waypointYaw;
    ...
    const msg = {
      ...
      longitude: lon,
      altitude: altitude,
      // Store yaw in position_covariance[8] as a non-standard extension
      // Yaw is in degrees (0-360, 0=North)
      position_covariance: [0, 0, 0, 0, 0, 0, 0, 0, yawDegrees],
      position_covariance_type: 0,
    };

    try {
      context.publish(SELECTED_WAYPOINT_TOPIC, msg);
      console.log("[GeofenceMap] ✓ Published:", lat, lon, altitude, `Yaw: ${yawDegrees}°`);
      setStatus(`Waypoint: ${lat.toFixed(6)}, ${lon.toFixed(6)}, Alt: ${altitude}m, Yaw: ${yawDegrees.toFixed(1)}°`);
    } catch (error) {
      console.error("[GeofenceMap] ✗ Publish failed:", error);
    }
  }, [context, SELECTED_WAYPOINT_TOPIC, waypointAltitude, waypointYaw]);
  ...
  const publishClickedObstacle = useCallback((lat: number, lon: number, radius: number) => {
    ...
    // Use sensor_msgs/NavSatFix format with radius stored in altitude field
    const msg = {
      header: {
        stamp: { sec: 0, nsec: 0 },
        frame_id: "obstacle",
      ...
      status: {
        status: -1,  // Use -1 to indicate this is an obstacle, not a waypoint
        service: 1,
      },
      latitude: lat,
      longitude: lon,
      altitude: radius,  // Store radius in altitude field for obstacles
  ...
  // Update selected waypoint marker with direction arrow
  const updateSelectedWaypointMarker = useCallback((lat: number, lon: number, yaw: number) => {
    ...
    // Create waypoint marker with direction arrow
    const icon = L.divIcon({
      className: "waypoint-marker",
      html: `<div style="background-color: #0000ff; width: 18px; height: 18px; border-radius: 50%; border: 2px solid white; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 10px; box-shadow: 0 1px 4px rgba(0,0,0,0.5); position: relative;">
        <div style="position: absolute; top: -12px; left: 50%; transform: translateX(-50%) rotate(${yaw}deg); color: #0000ff; font-size: 16px; text-shadow: 0 0 2px white, 0 0 2px white;">▲</div>
        W
      </div>`,
      iconSize: [18, 18],
      iconAnchor: [9, 9],
    ...
    const marker = L.marker([lat, lon], { icon })
      .addTo(map)
      .bindPopup(
        `<b>Selected Waypoint</b><br>Lat: ${lat.toFixed(6)}<br>Lon: ${lon.toFixed(6)}<br>Yaw: ${yaw.toFixed(1)}°<br><br><i>Click "Navigate to Waypoint" in Behavior Tree Controller</i>`
      );
  ...
  // Publish selected waypoint when it changes or altitude/yaw changes
  useEffect(() => {
    if (selectedWaypoint) {
      publishSelectedWaypoint(selectedWaypoint.lat, selectedWaypoint.lon);
      // Update marker with current yaw
      updateSelectedWaypointMarker(selectedWaypoint.lat, selectedWaypoint.lon, waypointYaw);
    }
  }, [selectedWaypoint, waypointAltitude, waypointYaw, publishSelectedWaypoint, updateSelectedWaypointMarker]);
    ...
    for (const messageEvent of messages) {
      ...
      try {
        ...
        if (topic === PLANNED_PATH_TOPIC) {
          ...
          plannedPathMarkersRef.current.forEach(marker => map.removeLayer(marker));
          plannedPathMarkersRef.current = [];
          ...
          if (pathMsg.poses && Array.isArray(pathMsg.poses) && pathMsg.poses.length > 1) {
            ...
            // Draw markers for each waypoint in the path
            latlngs.forEach((latlng: LatLngExpression, index: number) => {
                ...
                waypointMarker.bindPopup(`Path Waypoint #${index + 1}`);
                plannedPathMarkersRef.current.push(waypointMarker);
            });
        ...
        // Handle mapping completion to highlight the last casualty
        if (topic === GEOFENCE_MAPPING_STATUS_TOPIC) {
          const statusMsg = message as any; // Type is behavior_tree_msgs/Status
          if (statusMsg.status === 2) { // Status 2 means mapping is complete
            console.log("[GeofenceMap] 📍 Mapping complete. Highlighting last casualty.");
            const lastCasualtyId = Array.from(humanMarkersRef.current.keys())
              .filter(id => id.startsWith(CASUALTY_GEOLOCATED_TOPIC))
              .sort((a, b) => parseInt(a.split('-')[1] || '0') - parseInt(b.split('-')[1] || '0')) // Sort by timestamp
              .pop(); // Get the last one

            if (lastCasualtyId && humanMarkersRef.current.has(lastCasualtyId)) {
              const lastCasualtyMarker = humanMarkersRef.current.get(lastCasualtyId);
              if (lastCasualtyMarker) {
                lastCasualtyMarker.setIcon(L.divIcon({
                  className: "custom-marker",
                  html: `<div style="background-color: #00FF00; width: 20px; height: 20px; border-radius: 50%; border: 2px solid white; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 12px;">C</div>`,
                  iconSize: [20, 20],
                  iconAnchor: [10, 10],
                }));
                lastCasualtyMarker.setPopupContent(`<b>Last Casualty</b><br>Lat: ${lastCasualtyMarker.getLatLng().lat.toFixed(6)}<br>Lon: ${lastCasualtyMarker.getLatLng().lng.toFixed(6)}`);
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "8px", backgroundColor: "#f0f0f0", borderBottom: "1px solid #ccc" }}>
        ...
        <div style={{ 
          ...
          />
          <label style={{ fontWeight: "600", color: "#333", marginLeft: "12px" }}>
            Yaw (°):
          </label>
          <input
            type="number"
