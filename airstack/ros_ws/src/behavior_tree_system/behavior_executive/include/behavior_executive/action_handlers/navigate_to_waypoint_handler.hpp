...
public:
    ...
    /**
     * @brief Computes a safe path considering obstacles and optional polygon constraint.
     * 
     * This function implements the RRT* algorithm to compute a multi-point path
     * from a start to a goal location, avoiding a given list of obstacles and
     * staying within an optional polygon boundary (geofence).
     * 
     ...
     * @param polygon_vertices Optional polygon vertices (CW order) defining the geofence boundary.
    ...
    std::vector<sensor_msgs::msg::NavSatFix> compute_waypoint_path(
        ...
        const std::vector<GPSPoint>& polygon_vertices = std::vector<GPSPoint>()
    );
